...
def p_train_recurrent(make_obs_ph_n, make_state_ph_n, make_obs_next_n,
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        p, state = p_policy(obs_input, state_input, int(act_pdtype_n[p_index].param_shape()[0]), scope="p_policy", num_units=num_units)
        # wrap parameters in distribution
        act_pd = act_pdtype_n[p_index].pdfromflat(p)
        act_sample = act_pd.sample()                            # sample an action
        # predict the next obs
        obs_pred = p_predict(act_input, state, int(obs_input.shape[1]), scope="p_predict", num_units=num_units)
        ...
        loss = pg_loss + p_reg * 1e-3 + pred_loss * 1e-1
        ...
        target_p, target_state = p_policy(obs_input, state_input, int(act_pdtype_n[p_index].param_shape()[0]),
                                          scope="target_p_policy", num_units=num_units)

        target_obs_pred = p_predict(act_input, target_state, int(obs_input.shape[1]),
